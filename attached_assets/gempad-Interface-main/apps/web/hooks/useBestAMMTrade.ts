...
} from '@dapp/smart-router/evm';
import { CurrencyAmount, TradeType, Currency } from '@dapp/sdk';
import { ChainId } from '@dapp/chains';
import { useDebounce, usePropsChanged } from '@dapp/hooks';
...
export function useBestAMMTrade({ type = 'quoter', ...params }: useBestAMMTradeOptions) {
    ...
    const isQuoterAPIEnabled = useMemo(
        () => Boolean(!isWrapping && type === 'api'),
        [isWrapping, type],
    );

    const apiAutoRevalidate =
        typeof autoRevalidate === 'boolean' ? autoRevalidate : isQuoterAPIEnabled;
    ...
    const quoterAutoRevalidate =
        typeof autoRevalidate === 'boolean' ? autoRevalidate : isQuoterEnabled;
    ...
    return function useBestTrade({
        ...
        const {
            ...
            queryFn: async () => {
                ...
                const label = `[BEST_AMM](${key}) chain ${
                    currency.chainId
                }, ${deferAmount.toExact()} ${amount.currency.symbol} -> ${
                    currency.symbol
                }, tradeType ${tradeType}`;
                ...
                const res = await getBestTrade(deferAmount, currency, tradeType, {
                    gasPriceWei:
                        typeof gasPrice === 'bigint'
                            ? gasPrice
                            : async () =>
                                  publicClient({ chainId: amount.currency.chainId }).getGasPrice(),
                    maxHops,
                    poolProvider,
                    maxSplits,
                    quoteProvider,
...
function useQuoteProvider(chainId?: ChainId) {
    ...
    return useMemo(
        () => SmartRouter.createQuoteProvider({ onChainProvider: getViemClients, gasLimit }),
        [gasLimit],
    );
}
    ...
    getBestTrade: async (
        ...
        const serverRes = await fetch(`${QUOTING_API}`, {
            ...
            body: JSON.stringify({
                ...
                gasPriceWei:
                    typeof gasPriceWei !== 'function' ? gasPriceWei?.toString() : undefined,
                maxHops,
                maxSplits,
...
const createWorkerGetBestTrade = (quoteWorker: typeof worker): typeof SmartRouter.getBestTrade => {
    return async (
        ...
        const quoterConfig = (
            quoteProvider as ReturnType<typeof SmartRouter.createQuoteProvider>
        )?.getConfig();
