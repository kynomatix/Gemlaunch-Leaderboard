// import { getPoolContractBySousId } from '@dapp/pools'
...
import { WNATIVE, pancakePairV2ABI } from '@dapp/sdk';
import { ChainId } from '@dapp/chains';
import { CAKE } from '@dapp/tokens';
// import { nonfungiblePositionManagerABI } from '@dapp/v3-sdk'
...
import { PrivateSaleFactoryABI } from '@/config/abi/privateSaleFactory';
import { ServiceReceiverABI } from '@/config/abi/serviceReceiver';
import { AirdropFactoryABI } from '@/config/abi/airdropFactory';
import { uniswapV2PairABI } from '@/config/abi/uniswapV2PairABI';
import { FairLaunchFactoryABI } from '@/config/abi/fairlaunchFactory';
import { DutchActionFactoryABI } from '@/config/abi/dutchAuctionFactory';
import { subscriptionFactoryABI } from '@/config/abi/subscriptionFactory';
...
import { PrivateSaleAbi } from '@/config/abi/privateSale';
...
export const useStandardAntibot = (address: Address) => {
    return useContract(address, StandardAntibotTokenABI);
};

export const useCreateTokenContract = (address: Address, ABI: Abi) => {
    return useContract(address, ABI);
};

export const useCreateFairLaunchContract = (address: Address) => {
    return useContract(address, FairLaunchFactoryABI);
};
export const useFairLaunchContract = (address: Address) => {
    return useContract(address, FairLaunchABI);
};

export const useCreateDutchAuctionContract = (address: Address) => {
    return useContract(address, DutchActionFactoryABI);
};
export const useDutchAuctionContract = (address: Address) => {
    return useContract(address, DutchAuctionABI);
};

export const useCreateLaunchpadFactoryContract = (address: Address) => {
    return useContract(address, LaunchpadFactoryABI);
};

export const useLaunchpadContract = (address: Address) => {
    return useContract(address, LaunchpadABI);
};

export const useCreateAntibotContract = (address: Address) => {
    return useContract(address, AntibotABI);
};

export const useMultisenderContract = (address: Address) => {
    return useContract(address, MultisenderABI);
};

export const useLockerContract = (address: Address) => {
    return useContract(address, LockerABI);
};

// factory
export const usePrivateSaleFactory = (address: Address) => {
    return useContract(address, PrivateSaleFactoryABI);
};

// contract
export const usePrivateSaleContract = (address: Address) => {
    return useContract(address, PrivateSaleAbi);
};

export const useAirdropFactory = (address: Address) => {
    return useContract(address, AirdropFactoryABI);
};

export const useAirdropContract = (address: Address) => {
    return useContract(address, AirdropABI);
};

export const useSubscriptionFactoryContract = (address: Address) => {
    return useContract(address, subscriptionFactoryABI);
};
export const useSubscriptionContract = (address: Address) => {
    return useContract(address, SubscriptionABI);
};

export const useServiceReceiverContract = (address: Address) => {
    return useContract(address, ServiceReceiverABI);
};
...
export const usePredictionsContract = (address: Address, tokenSymbol: string) => {
    ...
    return useMemo(() => {
        ...
        const getPredContract =
            tokenSymbol === 'CAKE' ? getCakePredictionsContract : getPredictionsV2Contract;
...
export function usePairTokenContract(tokenAddress?: Address) {
    return useContract(tokenAddress, uniswapV2PairABI);
}
...
export const useCrossFarmingProxy = (proxyContractAddress: Address) => {
    ...
    return useMemo(
        () =>
            ...
            getCrossFarmingProxyContract(proxyContractAddress, signer, chainId),
...
export const useTradingRewardContract = ({ chainId: chainId_ }: { chainId?: ChainId } = {}) => {
    ...
    const { data: signer } = useWalletClient();
    return useMemo(
        () => getTradingRewardContract(signer, chainId_ ?? chainId),
...
export const useAffiliateProgramContract = ({ chainId: chainId_ }: { chainId?: ChainId } = {}) => {
    ...
    const { data: signer } = useWalletClient();
    return useMemo(
        () => getAffiliateProgramContract(signer, chainId_ ?? chainId),
        [signer, chainId_, chainId],
    );
};

export const useTradingRewardTopTraderContract = ({
    chainId: chainId_,
}: { chainId?: ChainId } = {}) => {
    const { chainId } = useActiveChainId();
    const { data: signer } = useWalletClient();
    return useMemo(
        () => getTradingRewardTopTradesContract(signer, chainId_ ?? chainId),
        [signer, chainId_, chainId],
    );
};
...
export const useVCakeContract = ({ chainId: chainId_ }: { chainId?: ChainId } = {}) => {
    ...
    const { data: signer } = useWalletClient();
    return useMemo(
        () => getVCakeContract(signer, chainId_ ?? chainId),
        [signer, chainId_, chainId],
    );
};

export const useRevenueSharingPoolContract = ({
    chainId: chainId_,
}: { chainId?: ChainId } = {}) => {
    const { chainId } = useActiveChainId();
    const { data: signer } = useWalletClient();
    return useMemo(
        () => getRevenueSharingPoolContract(signer, chainId_ ?? chainId),
        [signer, chainId_, chainId],
    );
};
...
export const useAnniversaryAchievementContract = ({
    ...
    const { data: signer } = useWalletClient();
    return useMemo(
        () => getAnniversaryAchievementContract(signer, chainId_ ?? chainId),
        [signer, chainId_, chainId],
    );
};
