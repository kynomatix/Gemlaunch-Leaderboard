...
import { calculateRemainingBuyLimit, getStatusString } from '../../utils';
...
const AuctionPresSaleTimer = ({ launchpadAddress }: { launchpadAddress: Address }) => {
    ...
    const { data: userInfo, isLoading: userInvestmentLoading } = useContractRead({
    ...
    const userTokens = userInfo?.[0] ?? 0n;
    const userInvest = userInfo?.[1] ?? 0n;
    const userClaimed = userInfo?.[2] ?? 0n;
    const userLastClaimTime = userInfo?.[3] ?? 0n;
    ...
    const loading = isLoading || userInvestmentLoading || fundTokenLoading;
    ...
    // const hasUserClaimedAllToken = (Number(Number(userClaimed) + Number(userShare)) >= Number(userShare))
    ...
    const remainingBuyLimit = calculateRemainingBuyLimit(userInvestment, Number(maxLimit));
    ...
    const hasEnoughBalanceToBuy = userFundTokenBalance >= minLimit;

    // const balanceLowerToReminingLimit = userFundTokenBalance >= remainingBuyLimit
                        ...
                        <>
                            ...
                            >
                                {totalRaisedNum < softCapNum ? null : (
                                    ...
                                    >
                                        {/* {hasUserClaimedAllToken ? "Claimed" : isClaimIdle ? `Claim ${tokenToReceive}` : null} */}
                                        {isClaimIdle && `Claim`}
...
export default AuctionPresSaleTimer;
