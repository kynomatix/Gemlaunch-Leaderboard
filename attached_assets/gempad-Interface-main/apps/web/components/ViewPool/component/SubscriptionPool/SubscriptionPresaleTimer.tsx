...
import { Box, Button, InputAdornment, Link, Skeleton, TextField, Typography } from '@mui/material';
...
const SubscriptionPreSaleTimer = ({ launchpadAddress }: { launchpadAddress: Address }) => {
    ...
    const userInvestment = formatUnits(userInvest, fundTokenDecimals ?? 18);

    // subscription pool did'nt have minBuylimit for validation purpose we need to set it for zero
    const minLimit = +formatUnits(0n, fundTokenDecimals ?? 18);
    ...
    const tokenToReceive = `${buyAmount * Number(formatUnits(sellRate, tokenDecimals ?? 18))
        } ${symbol}`;
    ...
    const buyWithToken = async (buyAmount) => {
        ...
        try {
            ...
            const estimationGas = await buyTokenGas(
                [parseUnits(buyAmount.toString(), fundTokenDecimals)],
                {
                    // value: parseUnits(buyAmount.toString(), fundTokenDecimals),
                },
            );

            const hash = await buyToken([parseUnits(buyAmount.toString(), fundTokenDecimals)], {
                // value: parseUnits(buyAmount.toString(), fundTokenDecimals),
    ...
    const buyWithNativeCurrency = async (buyAmount) => {
        ...
        try {
            ...
            const estimationGas = await buyTokenGas([amountInWei], {
                value: parseUnits(buyAmount.toString(), 18),
            });

            const hash = await buyToken([amountInWei], {
                value: parseUnits(buyAmount.toString(), 18),
                gas: estimationGas || DefaultGasLimit,
            });
    ...
    const canRefund = useMemo(() => {
        const refund =
            (moment().isSameOrAfter(endTimeMoment) && totalContributionNum < softCapNum) ||
            currentLaunchpadStatus === LaunchpadSaleStatus.CANCELLED;
        return refund;
    }, [currentLaunchpadStatus, endTimeMoment, softCapNum, totalContributionNum]);
    ...
    React.useEffect(() => {
        refetch();
        forceUpdate();
    }, [address, isReconnecting, refetch, forceUpdate]);
    ...
    return (
        <>
            ...
            /> : <PrimaryCard py={25}>
                ...
                <Box sx={{ mt: '10px' }}>
                    <LinearProgressCustom
                        bgColor="#BFFFD5"
                        value={getProgress(totalContributionNum, hardCapNum)}
                    />
                    <Box
                        sx={{
                            display: 'flex',
                    ...
                    >
                        <Typography variant="h5" fontSize={12}>
                            {totalContributionNum} {fundTokenSymbol}
                        </Typography>
                    ...
                    <Box sx={{ mt: '20px' }}>
                        ...
                        >
                            <Typography fontSize={12} fontWeight={600} mb={1}>
                                Max: {remainingBuyLimit} {fundTokenSymbol ?? '??'}
                            </Typography>
                            <Typography fontSize={12} fontWeight={600} mb={1}>
                ...
                {isSaleClosed && (
                    <>
                        ...
                        >
                            {totalContributionNum < softCapNum ? null : (
                                <Button
                                    variant="contained"
                                    onClick={() => claimNow(ClaimType.TOKEN)}
                                    disabled={
                                        isClaimPending || isClaimProcessing || userClaimed > 0n
                                    }
                                    size="small"
                                    sx={{ width: '100%', marginBottom: '5px' }}
                                >
                                    {isClaimIdle && userClaimed === 0n
                                        ? 'claim'
                                        : userClaimed > 0n
                                            ? 'Claimed'
                                            : null}
                ...
                {isSaleCancelled && canRefund && userInvest > 0n && (
                    <Button
                        variant="outlined"
                        onClick={() => refund()}
                        ...
                        sx={{ width: '100%' }}
                    >
                        {isClaimIdle && `Refund (${userInvestment} ${fundTokenSymbol})`}
                        {isClaimPending && <ButtonLoader text="Pending..." />}
                        {isClaimProcessing && <ButtonLoader text="Processing..." />}
                    </Button>
                )}
            ...
            <HistoryModal
                ...
                onClose={handleClose}
                subscriptionPoolAddress={launchpadAddress}
                fundTokenDecimals={fundTokenDecimals}
                fundTokenSymbol={fundTokenSymbol}
                launchpadTokenSymbol={symbol}
                launchpadTokenDecimals={tokenDecimals}
                sellRateNum={sellRateNum}
            />
        </>
    );
};

export default SubscriptionPreSaleTimer;
