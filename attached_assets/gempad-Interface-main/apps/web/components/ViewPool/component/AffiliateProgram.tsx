...
const AffiliateProgram = ({
    ...
    // according to to formula used in solidity
    const fee = (totalRaisedNum * tokenFee) / 100;
    const total = ((totalRaisedNum - fee) * affiliatePercentage) / 100;
        ...
        const reward =
            allUsersRewardInfo && allUsersRewardInfo?.length > 0
                ? allUsersRewardInfo?.reduce((acc, curr, ind) => {
                    ...
                    const userReward = (total * userReferralInvest) / referralsInvestment;
    ...
    }, [allUsersRewardInfo, fundTokenDecimals, total, referralsInvestment]);
    ...
    const { result: claimTime } = useSingleCallResult({
    ...
    const claimNow = async () => {
        ...
        if (currentTime <= Number(claimTime)) {
