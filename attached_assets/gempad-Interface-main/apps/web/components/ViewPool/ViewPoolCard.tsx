...
import { getLaunchpadRouteByContractName } from './utils';
...
interface ViewPoolCardProps {
    progress: number;
    status: Status;
    url: StaticImageData;
    isAffiliate: boolean;
    badgeUrl: StaticImageData;
}
...
const ViewPoolCard = (props: LaunchPad) => {
    const {
        token,
        minBuyLimit,
        maxBuyLimit,
    ...
    const hardCaps = +formatUnits(hardCap ?? 0n, fundTokenDecimals ?? 18);
    const softCaps = +formatUnits(softCap ?? 0n, fundTokenDecimals ?? 18);
    // TODO: show HardCap and softCap
    ...
    const totalProgress = isFairLaunch
        ? formatUnits(softCap, fundToken?.decimals ?? 18)
        : formatUnits(hardCap, fundToken?.decimals ?? 18);
            ...
            <PrimaryCard py={35}>
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                        ...
                        <Link
                            href={
                                contractAddress && launchpadName
                                    ? `/${getLaunchpadRouteByContractName(
                                          launchpadName,
                                          contractAddress,
                                      )}`
                                    : '/404'
