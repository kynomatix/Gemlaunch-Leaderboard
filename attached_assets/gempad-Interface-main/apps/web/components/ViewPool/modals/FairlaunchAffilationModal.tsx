...
interface FairLaunchAffilationModalProps extends ModalProps {
  contract: any;
}

const FairLaunchAffilationModal = (props: FairLaunchAffilationModalProps) => {
  const { handleClose, open, contract } = props;
  const [tx, setTx] = React.useState<Tx>(Tx.Idle);

  const { chainId } = useActiveChainId();

  const { affiliatePercentage } = useFairLaunchDetails(contract.address)

  const affililiateReward = affiliatePercentage;
...
export default FairLaunchAffilationModal;
