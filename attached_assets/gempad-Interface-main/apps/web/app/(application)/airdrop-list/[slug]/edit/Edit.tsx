...
import { EditAirdropsFormInput, Tx, Upload } from '@/components/CreateAirdrop/types';
import { yupResolver } from '@hookform/resolvers/yup';
import * as yup from 'yup';
import { isValidUrl } from '@/utils/isValidUrl';
import { EDIT_AIRDROP_VALIDATION, Labels } from '@/components/CreateAirdrop/validation';
import PrimaryCard from '@/components/Cards/PrimaryCard';
import { useParams, useRouter } from 'next/navigation';
import { OperationVariables, useMutation, useQuery } from '@apollo/client';
import { GET_SINGLE_AIRDROP } from '@/components/ExploreAirdrops/query';
import { useActiveChainId } from '@/hooks/useActiveChainId';
import { EDIT_AIRDROP, EDIT_AIRDROP_METADATA } from '@/components/CreateAirdrop/query';
...
const Edit = () => {
    ...
    const { data } = useQuery<{ airdrops: Airdrop[] }, OperationVariables>(EDIT_AIRDROP, {
        variables: {
    ...
    const [editAirdropMetadata] = useMutation(EDIT_AIRDROP_METADATA, {
        client,
    });
    ...
    const {
        ...
        resolver: yupResolver(EDIT_AIRDROP_VALIDATION) as any,
    });

    React.useEffect(() => {
        if (!data?.airdrops?.[0]?.metadata?.socials) return;
        ...
        const {
            ...
            twitterUrl,
            youtubeUrl,
        } = data.airdrops[0].metadata.socials;
    ...
    const onSubmit: SubmitHandler<EditAirdropsFormInput> = async (data) => {
        try {
            setTx(Tx.WAITING);
            ...
            toast.success(<DescriptionWithTx title="Success" description="Airdrop updated" />);
            router.back();
        } catch (e: any) {
            console.error(e);
            toast.error(
