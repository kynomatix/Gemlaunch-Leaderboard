import { Currency, CurrencyAmount, Native, Token } from '@dapp/sdk';
...
export function useNativeBalances(uncheckedAddresses?: (string | undefined)[]): {
    ...
    const addresses: Address[] = useMemo(
        () =>
            uncheckedAddresses
                ? orderBy(
                      uncheckedAddresses.map(isAddress).filter((a): a is Address => a !== false),
                  )
                : [],
        [uncheckedAddresses],
    );
            ...
            () =>
                address && validatedTokens.length > 0
                    ? validatedTokens.reduce<{
                          ...
                          const amount =
                              typeof value !== 'undefined' ? BigInt(value.toString()) : undefined;
